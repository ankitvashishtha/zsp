...
app.controller('part13Ctrl', function ($scope, $http, $state) {
    ...
    $scope.yearName = sessionStorage.yearName;
    var entryMethodName = 'entry/getEntryForm';
    if (sessionStorage.tableNo == '13.1')
        entryMethodName = 'entry/getEntryForm13_1';
    $scope.getEntryForm = function () {
        $scope.isLoading = true;
        $http({
            method: "GET",
...
app.controller('part15Ctrl', function ($scope, $http, $state) {
    ...
    $scope.yearName = sessionStorage.yearName;
    var entryMethodName = 'entry/getEntryForm';
    var editMethodName = 'entry/getEntryForm';
    if (sessionStorage.tableNo == '15.1') {
        entryMethodName = 'entry/getEditForm15';
        editMethodName = 'entry/getEditForm15_1';
    }
    $scope.getEntryForm = function () {
        $scope.isLoading = true;
        $http({
    ...
    $scope.getEditForm = function () {
        ...
        $http({
            method: "GET",
            url: baseUrl + 'api/' + editMethodName,
            params: { indexId: sessionStorage.indexId }
        }).then(function mySucces(response) {
            $scope.index = response.data[0];
            $scope.formTable = response.data[1];
            $scope.emptyRow = angular.copy($scope.formTable[0]);
            $scope.emptyRow.id = 0;
            $scope.emptyRow.NewsPaperName = null;
            $scope.emptyRow.Languages = null;
            $scope.emptyRow.TimePeriod = null;
            $scope.emptyRow.Regularity = null;
            $scope.isLoading = false;
        }, function myError(response) {
            $.prompt("Error Get form");
        });
    ...
    $scope.saveTable = function (tableNo) {
        ...
        var data = [$scope.index, $scope.formTable];
        var saveMethodName = 'entry/saveTable15';
        if (sessionStorage.tableNo == '15.1')
            saveMethodName = 'entry/saveTable15_1';
        var totalError = false;
        if (tableNo != 1) {
            totalError = $scope.totalCheck();
        }
        if ($scope.IsFormValid) {
            if (!totalError) {
                $http({
                    method: "POST",
                    url: baseUrl + "api/" + saveMethodName,
                    data: data
                }).then(function mySucces(response) {
                    $.prompt(response.data);
                    $state.go('entry');
